...
const ProductPage = () => {
  ...
  const [localQauntity, setLocalQuantity] = useState(1)
  ...
  const displayQuantity = inCart ? cartItem.quantity :  localQauntity
  ...
  const handleMinus = () =>{
    if(inCart){
      ...
      setLocalQuantity(Math.max(1, localQauntity-1))
    }
  }
  ...
  const handlePlus = () =>{
    if(inCart)
       updateQuantity(product._id, cartItem.quantity + 1)
        else setLocalQuantity(localQauntity + 1)
  }
           ...
           <div className="grid md:grid-cols-2 gap-0">
              ...
              md:min-h-[480px]">
               ...
               className="max-h-[360px] w-auto object-contain"/>
                ...
                {product.isOrganic && (
                  <span className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-app-green text-white
                  rounded-full">
              ...
              <div className="p-6 md:p-10 flex flex-col justify-center">
                <span className="text-xs font-medium text-app-text-light tracking-wider 
                mb-2 capitalize">
                {categoryLabel}
                </span>
                ...
                <div className="flex items-baseline gap-3 mb-5">
                  ...
                  <p className="text-sm text-app-text-light leading-relaxed mb-6">{product.description}</p>
                   ...
                   <div className="flex items-center gap-3">
                       ...
                       <button
                       onClick={()=>{
                        if(!inCart) addToCart(product, localQauntity)
                       }}
                       disabled={product.stock === 0} 
                       className={`flex-1 py-3 font-semibold rounded-xl transition-colors 
                        flex-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]
                         ${inCart ? "bg-app-cream text-app-green border border-app-green" : 
         ...
         {relatedProducts.length > 0 &&(
          <section className="mt-12 mb-44">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link className="text-sm fotn-semibold text-app-orange hover:text-app-orange-dark 
