             ...
             <select
                    ...
                    rounded-xl border border-app-border focus:border-app-green
                     ...
                     <option value="Price_desc">Price: High to Low</option>
                      <option value="Name_asc">Top Rated</option>
                       <option value="Name_desc">Name: A to Z</option>
                        </select>
                        <ChevronDown className="absolute right-2.5 
                        top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-app-text-light pointer-events-none"/>
               </div>
             </div>
            </div>
          {/* Products Grid */}
          {loading ? (
            <Loading />
              ...
              <div className="text-center py-16">
                  ...
                  <button onClick={clearFilters} className="px-5 py-2 text-sm font-medium bg-app-green text-white rouded-xl
                  hover:bg-app-green-light transition-colors">
                    Clear Filters
                  </button>
              </div>
            ):(
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols gap-4 xl:gap-8">
          ...
          {totalPages > 1 && (
            <div className="flex-center gap-2 mt-16">
                {Array.from({length: totalPages}).map((_, i)=>(
                  <button className={`size-9 rounded-lg text-sm font-medium transition-colors ${page === i + 1 ? "bg-app-green text-white" : "bg-white text-app text-app-text-light hover:bg-app-cream"}`}>
                    {i + 1}
                  </button>
                ))}
            </div>
